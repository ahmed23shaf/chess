...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//write reg + 2 via SPI
	//write val via SPI
	BYTE data_arr[2] = {reg+2, val}; // {COMMAND_BYTE, DATA}

	ret = XSpi_Transfer(&SpiInstance, data_arr, NULL, 2);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE data_arr[1+nbytes]; // {COMMAND_BYTE, DATA}
	data_arr[0] = reg+2;

	for (int n = 0; n < nbytes; n++) // append the data into data_arr
	{
		data_arr[n+1] = data[n];
	}

	ret = XSpi_Transfer(&SpiInstance, data_arr, NULL, 1+nbytes);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//write reg via SPI
	//read val via SPI
	BYTE data_arr[2]; // {COMMAND_BYTE, DATA}
	data_arr[0] = reg;

	ret = XSpi_Transfer(&SpiInstance, data_arr, data_arr, 2);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	ret = XSpi_Transfer(&SpiInstance, data_arr, data_arr, 1+nbytes);

	for (int n = 0; n < nbytes; n++) // Copy over data from XSpi_Transfer() call
	{
		data[n] = data_arr[n+1];
	}
